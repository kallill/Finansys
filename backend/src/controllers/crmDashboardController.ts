...
export const getStats = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 2. Cálculo do MRR (Soma dos valores dos planos de todas as assinaturas ativas)
    ...
    // 3. Cálculo de Inadimplência (Soma dos valores das assinaturas com status 'Atrasado')
    ...
    // 4. Ordens de Serviço Abertas (Status != 'Concluída')
    ...
    // 5. Dados para o gráfico (Mockando histórico baseado no MRR atual para visualização)
    // Em um sistema real, teríamos uma tabela de histórico mensal
    const mrrHistory = [
