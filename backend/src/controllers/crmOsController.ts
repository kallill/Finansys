...
export const uploadAnexo = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!file) {
      res.status(400).json({ success: false, message: 'Nenhum arquivo recebido ou formato invÃ¡lido.' });
      return;
    }

    const os = await CRMOrdemServico.findByPk(id as string);
    if (!os) {
      res.status(404).json({ success: false, message: 'Ordem de serviÃ§o nÃ£o encontrada.' });
      return;
    }

    // Como o multer salvou na pasta local exposta pelo nginx, o caminho pÃºblico serÃ¡:
