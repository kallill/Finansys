...
export const uploadAnexo = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!file) {
      res.status(400).json({ success: false, message: 'Nenhum arquivo recebido ou formato inválido.' });
      return;
    }
    ...
    // Como o multer salvou na pasta local exposta pelo nginx, o caminho público será:
