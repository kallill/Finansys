...
const Cards = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError('Falha ao carregar cartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âµes');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setError('Erro ao salvar cartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o');
    }
  };

  const onDelete = async (id) => {
    if (!window.confirm('Tem certeza que deseja excluir este cartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o?')) return;
    try {
      await deleteCreditCard(id);
      await load();
    } catch {
      setError('Erro ao excluir cartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o');
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white p-6 md:p-8">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold flex items-center gap-2 px-3 py-1 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-lg">
              <CreditCard size={24} /> Meus CartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âµes
            </h1>
          </div>
          <Button variant="primary" onClick={openCreate} className="shadow-lg shadow-emerald-500/20">
            <Plus size={18} /> Novo CartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            <div className="col-span-full text-center py-20 text-slate-500">Buscando seus cartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âµes...</div>
          ) : cards.length === 0 ? (
            <div className="col-span-full bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-800 rounded-3xl p-12 text-center">
               ...
               <p className="text-slate-500 dark:text-slate-400">Nenhum cartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o cadastrado ainda.</p>
               <button onClick={openCreate} className="text-emerald-500 font-medium mt-2 hover:underline">Adicionar meu primeiro cartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o</button>
            </div>
          ) : (
            cards.map(card => {
                ...
                <div key={card.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400">CrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©dito</p>
                    </div>
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 w-full max-w-md shadow-2xl">
              <h2 className="text-xl font-bold mb-6 text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {editing ? 'Editar CartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o' : 'Novo CartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o'}
              </h2>
              <form className="space-y-4" onSubmit={onSubmit}>
                <div>
                  <label className="text-xs font-semibold text-slate-500 ml-2 mb-1 block">Nome do CartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o/Banco</label>
                ...
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  <button type="submit" className="px-6 py-3 rounded-2xl bg-emerald-500 text-white font-bold shadow-lg shadow-emerald-500/20 hover:scale-[1.02] active:scale-95 transition-all">
                    {editing ? 'Salvar AlteraÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âµes' : 'Criar CartÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o'}
                  </button>
                </div>
              </form>
            </div>
