...
const Cards = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError('Falha ao carregar cartÃƒÂµes');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setError('Erro ao salvar cartÃƒÂ£o');
    }
  };

  const onDelete = async (id) => {
    if (!window.confirm('Tem certeza que deseja excluir este cartÃƒÂ£o?')) return;
    try {
      await deleteCreditCard(id);
      await load();
    } catch {
      setError('Erro ao excluir cartÃƒÂ£o');
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white p-6 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            <div className="col-span-full text-center py-20 text-slate-500">Buscando seus cartÃƒÂµes...</div>
          ) : cards.length === 0 ? (
            <div className="col-span-full bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-800 rounded-3xl p-12 text-center">
               ...
               <p className="text-slate-500 dark:text-slate-400">Nenhum cartÃƒÂ£o cadastrado ainda.</p>
               <button onClick={openCreate} className="text-emerald-500 font-medium mt-2 hover:underline">Adicionar meu primeiro cartÃƒÂ£o</button>
            </div>
          ) : (
            cards.map(card => {
                ...
                <div key={card.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400">CrÃƒÂ©dito</p>
                    </div>
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 w-full max-w-md shadow-2xl">
              <h2 className="text-xl font-bold mb-6 text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {editing ? 'Editar CartÃƒÂ£o' : 'Novo CartÃƒÂ£o'}
              </h2>
              <form className="space-y-4" onSubmit={onSubmit}>
                <div>
                  <label className="text-xs font-semibold text-slate-500 ml-2 mb-1 block">Nome do CartÃƒÂ£o/Banco</label>
                ...
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  <button type="submit" className="px-6 py-3 rounded-2xl bg-emerald-500 text-white font-bold shadow-lg shadow-emerald-500/20 hover:scale-[1.02] active:scale-95 transition-all">
                    {editing ? 'Salvar AlteraÃƒÂ§ÃƒÂµes' : 'Criar CartÃƒÂ£o'}
                  </button>
                </div>
              </form>
            </div>
