...
const ReportsPage = () => {
  ...
  const exportCsv = () => {
    const header = ['DescriÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o', 'Categoria', 'Tipo', 'Valor', 'Data'];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white p-6 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold flex items-center gap-2"><PieIcon /> RelatÃƒÆ’Ã‚Â³rios</h1>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <input className="bg-slate-100 dark:bg-slate-800 rounded-xl p-3" type="date" value={filters.start} onChange={e => setFilters({ ...filters, start: e.target.value })} placeholder="InÃƒÆ’Ã‚Â­cio" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left border-b border-slate-200 dark:border-slate-800">
                <th className="p-4">DescriÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o</th>
