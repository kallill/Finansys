...
const WhatsAppCard = () => {
  ...
  const handleDisconnect = async () => {
    if (window.confirm('Deseja realmente desconectar seu WhatsApp do rob̫?')) {
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-sm relative overflow-hidden group">
      ...
      <div className="flex items-center gap-4 mb-6 relative z-10">
        ...
        <div className="flex-1">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white leading-none mb-1">C̩rebro WhatsApp</h3>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
              {isConnected ? 'Conex̣o Ativa' : 'Desconectado'}
            </span>
          </div>
        </div>
      </div>
      ...
      {isConnected ? (
        ...
        <div className="relative z-10">
           <p className="text-xs text-slate-500 mb-5 leading-relaxed">
             Conecte seu WhatsApp para registrar transa̵̤es usando sua voz ou texto natural com IA.
           </p>
           <Button 
             variant="primary" 
